...
const SS_ID = '1hKeM-13t6wyGD5Ya4Rx9NeUJXsgJoVN4dOb-rklPznA'; // PRODUCCIÓN
  ...
  try {
    ...
    if (payload.idVenta) {
      ...
      try {
        ...
        if (procesados2.indexOf(payload.idVenta) !== -1) {
          // v19/09/2026 — FIX RAÍZ: devolver también el ticket real ya
          // generado (ver guardado más abajo, en PASO 3) — antes esta
          // respuesta no lo incluía, y eso hacía que el fiado se quedara
          // sin número de ticket cuando el reintento llegaba después de
          // que la venta original ya se había hecho de verdad.
          var ticketYaGenerado = null;
          try {
            var mapaTkLookup = JSON.parse(props.getProperty('ventas_ticket_por_id') || '{}');
            ticketYaGenerado = mapaTkLookup[payload.idVenta] || null;
          } catch (eLookupTk) {}
          return respuestaJSON({ success: true, duplicate: true, ticket: ticketYaGenerado });
        }
    ...
    try {
      ...
      if (sheetVentas) {
        ...
        // v19/09/2026 — FIX RAÍZ: guardar qué ticket generó este idVenta, para
        // poder devolverlo si el mismo idVenta vuelve a llegar (reintento del
        // celular tras un timeout). Sin esto, la detección de duplicado de
        // arriba respondía "ok, ya existía" pero SIN el número de ticket —
        // causa real de por qué la hoja FIADOS se quedaba sin ticket cuando
        // la venta original sí se había hecho, solo que el celular no llegó
        // a recibir la primera respuesta a tiempo.
        if (payload.idVenta) {
          try {
            var propsTk = PropertiesService.getScriptProperties();
            var mapaTk = {};
            try { mapaTk = JSON.parse(propsTk.getProperty('ventas_ticket_por_id') || '{}'); } catch(eParseTk) { mapaTk = {}; }
            mapaTk[payload.idVenta] = ticketStr;
            var clavesTk = Object.keys(mapaTk);
            if (clavesTk.length > 150) {
              clavesTk.slice(0, clavesTk.length - 150).forEach(function(k) { delete mapaTk[k]; });
            }
            propsTk.setProperty('ventas_ticket_por_id', JSON.stringify(mapaTk));
          } catch (eTkSave) { console.warn('⚠️ No se pudo guardar ticket_por_id:', eTkSave); }
        }
